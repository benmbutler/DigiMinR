...
The XRPD data were collected using Cu-Kα radiation [for details see @Butler2018] and are included within the **mars2mull** package in the `scotland_xrpd` data, which a data frame rather than a `multiXY` object in order to save on file size when transferring to/from GitHub.
...
1. Transform the 2θ scale of the `scotland_xrpd` data to its Co-Kα equivalent
2. Calculate the overlapping 2θ range of the two datasets
3. Interpolate the data onto a harmonised 2θ scale
